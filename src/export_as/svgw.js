...
function ExportAsSVGWhite(bounds) {
	...
	this.rect = function(x, y, width, height) {
		...
		var style = `stroke="${this.strokeStyle}" stroke-width="${this.lineWidth}" fill="none"`;
		this._svgData += `\t<rect ${style} x="${fixed(x, 3)}" y="${fixed(y, 3)}" width="${width}" height="${height}"/>\n`;
	};
	this.ellipse = function(x, y, hradius, vradius, rotation, startAngle, endAngle, isReversed) {
		...
		var style = `stroke="${this.strokeStyle}" stroke-width="${this.lineWidth}" fill="none"`;
		this._svgData += `\t<ellipse ${style} cx="${fixed(x, 3)}" cy="${fixed(y, 3)}" rx="${fixed(hradius, 3)}" ry="${fixed(vradius, 3)}"/>\n`;
	};
